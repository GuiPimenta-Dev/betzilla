...
export class IsPositiveEV implements Rule {
  constructor(private odds: Odd[], private guess: Guess, private side: Side, private betValue: number) {}
  ...
  private getMaxOdd(index: number) {
    const odds = this.odds[index][this.side];
    return Math.max(...odds);
  }

  private getFairOdd(odd: number, resultIndex: number) {
    const oddIndex = this.odds[resultIndex][this.side].indexOf(odd);
    const oppositeType = this.side === "back" ? "lay" : "back";
