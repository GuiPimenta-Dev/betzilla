...
test("It should schedule a verifyBet if bet was made in this match", async () => {
  ...
  const sut = new MatchFinishedHandler({ matchRepository, strategyRepository, broker: brokerSpy });
  const event = new MatchFinished(match.id);
  await sut.handle(event);

  expect(brokerSpy.scheduledCommands).toHaveLength(1);
  expect(brokerSpy.scheduledCommands[0].payload).toEqual({ betId: "betId", playerId: "playerId" });
...
test("It should finish a match and change the status to finished", async () => {
  ...
  const sut = new MatchFinishedHandler({ matchRepository, strategyRepository, broker });
  const event = new MatchFinished(match.id);
  await sut.handle(event);
