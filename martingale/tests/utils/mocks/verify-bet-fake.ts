...
export class VerifyBetFake implements Handler {
  ...
  async handle(input: VerifyBet): Promise<void> {
    ...
    const event =
      response.status === "won"
        ? new BetWon({ betId: payload.betId, amount: response.outcome })
        : new BetLost({ betId: payload.betId });
    await this.broker.publish(event);
    await this.broker.publish(new BetVerified({ betId: input.payload.betId, status: response.status }));
  }
}
