...
test("It should emit a send email command when martingale is finished", async () => {
  ...
  const martingaleFinished = new MartingaleFinished({ martingaleId: "default", status: "finished" });
  ...
  expect(brokerSpy.commands[0].payload.body).toBe('{"history":[],"profit":0,"status":"finished"}');
});

test("It should save the status in the martingale", async () => {
  const brokerSpy = new BrokerSpy(new InMemoryBroker());
  const martingaleRepository = new InMemoryMartingaleRepository();
  martingaleRepository.createDefaultMartingale();
  ...
  expect(martingale.status).toBe("finished");
});